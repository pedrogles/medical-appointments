...
export class AppointmentSheduleFormComponent implements OnInit {
  appointmentSheduleForm!: FormGroup<{
  ...
  initializeForm(): void {
    this.appointmentSheduleForm = this.formBuilder.nonNullable.group({
  ...
  onSubmit(): void {
    this.isLoading = true;
    // const appointmentSheduleData: CreateAppointmentDTO = this.buildCreateAppointmentDTO();
    setTimeout(() => {
      this.toast.show(`Consulta agendada com sucesso!`, 'success');
      this.isLoading = false;
      this.appointmentSheduleForm.reset();
    }, 2000); 
  }

  // private buildCreateAppointmentDTO(): CreateAppointmentDTO {
  //   const formValue = this.appointmentSheduleForm.getRawValue();
  //   return {
  ...
  get formControls(): typeof this.appointmentSheduleForm.controls {
      return this.appointmentSheduleForm.controls;
  }
}
